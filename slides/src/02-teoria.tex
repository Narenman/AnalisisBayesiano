...
\begin{frame}
  ...
  \begin{figure}
    \centering
    \begin{tikzpicture}[every node/.style={fill=Mulberry!50!white, shape=rectangle, minimum width=0.4\textwidth, minimum height=0.3\textheight, align=center, inner sep=4mm, node distance=4mm]}]
      ...
      \begin{scope}[on background layer]
        \node [fill=NavyBlue!50!white, fit={(resultados) (eventos) (variables) (probabilidades)}, label={[label distance=-8mm]above:\textbf{Experimento}}] (experimento) {};
      \end{scope}
    \end{tikzpicture}
  \end{figure}
\end{frame}
      ...
      \begin{figure}
        ...
        \begin{tikzpicture}
          ...
          \foreach \x in {1,...,6}{%
            \foreach \y in {1,...,6}{%
              \pgfmathsetmacro\z{int(\x+\y)}%
              \pgfmathsetmacro\ncola{and(\x <= 4, \y <= 4) ? "red": "white"}%
              \pgfmathsetmacro\ncolb{or(\x < 3, \x > 5) ? "red": "white"}%
              \pgfmathsetmacro\ncolc{\z >= 7 ? "red": "white"}%
              \pgfmathsetmacro\ncold{and(\z >=7, \y >= 3) ? "red": "white"}%
