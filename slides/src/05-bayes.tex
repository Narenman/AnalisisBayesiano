...
\begin{frame}
  ...
  Si tenemos observaciones $y$ de un observable $Y$ y un modelo parametrizado por $\Theta$, podemos calcular la distribución de los parámetros dadas las observaciones usando:
  \begin{equation*}
...
\begin{frame}
  ...
  \begin{overprint}
    ...
    \item $p(y \mid \theta)$: \textbf{Verosimilitud}\\
      Indica qué valores del observable $Y$ son más o menos posibles dado el valor específico $\Theta = \theta$ de los parámetros.
    ...
    \item $p(y) = \int p(y \mid \theta) p(\theta) \, \symrm{d} \theta$: \textbf{Verosimilitud marginal}\\
      Indica qué valores del observable $Y$ son más o menos posibles dada la información disponible \emph{a priori} acerca de los parámetros del modelo
    \end{itemize}
  \end{overprint}
\end{frame}
%
